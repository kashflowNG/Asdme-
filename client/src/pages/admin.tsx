                ...
                <TabsContent value="locations" className="p-4 sm:p-6">
                  <div className="space-y-6">
                    ...
                    <div className="space-y-3">
                      {filteredUsers.map((user) => {
                        const validLocations = user.locations?.filter(loc => 
                          loc.country && loc.country !== 'Unknown' && loc.country !== 'null'
                        ) || [];
                              ...
                              <div className="flex items-center gap-2 sm:ml-4">
                                {validLocations.length > 0 ? (
                                  <div className="flex flex-wrap gap-2">
                                    {validLocations.slice(0, 3).map((loc, idx) => (
                                      <div
                                        key={idx}
                    ...
                    <div className="mt-8">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        {(users as AdminUser[]).reduce((acc: any[], user) => {
                          if (!user.locations || user.locations.length === 0) return acc;
                          
                          user.locations.forEach(loc => {
                            // Filter out invalid locations
                            if (!loc.country || loc.country === 'Unknown' || loc.country === 'null') {
                              return;
                            }
                            ...
                            if (existing) {
                              ...
                              if (!existing.users.includes(user.username)) {
                      ...
                      {(users as AdminUser[]).reduce((acc: any[], user) => {
                        ...
                        <div className="text-center py-8">
                          ...
                          <p className="text-xs text-gray-500 mt-1">Location data will appear when users visit profiles</p>
                        </div>
                      )}
                    </div>
                  </div>
